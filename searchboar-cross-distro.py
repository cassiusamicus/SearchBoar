...
class SearchBoar(Gtk.Window):
    ...
    def _detect_package_manager(self):
        """Return a package-manager identifier and a human-readable label."""
        import shutil

        if shutil.which("apt-get") or shutil.which("apt"):
            return "apt", "Debian/Ubuntu/MX Linux"
        if shutil.which("pacman"):
            return "pacman", "Arch/Artix/EndeavourOS"
        if shutil.which("dnf"):
            return "dnf", "Fedora"
        if shutil.which("zypper"):
            return "zypper", "openSUSE"
        return "unknown", "Unknown distribution"

    def _build_dependency_install_command(self, missing):
        """Build the appropriate package-install command for this system."""
        manager, distro_label = self._detect_package_manager()
        ...
        if manager in package_maps:
            packages = [package_maps[manager][item] for item in missing]
            packages = list(dict.fromkeys(packages))
            joined = " ".join(packages)
            commands = {
                "apt": f"sudo apt update && sudo apt install {joined}",
                "pacman": f"sudo pacman -S {joined}",
                "dnf": f"sudo dnf install {joined}",
                "zypper": f"sudo zypper install {joined}",
            }
            return commands[manager], distro_label

        # Portable fallback. pdftotext and ripgrep still need system packages,
        # but pypdf can be installed in the user's Python environment.
        if missing == ["pypdf"]:
            return "python3 -m pip install --user pypdf", distro_label

        return (
            "Install ripgrep and Poppler with your system package manager, "
            "then run: python3 -m pip install --user pypdf",
            distro_label,
        )

    def check_dependencies(self):
        """Check optional dependencies and offer the correct install command."""
        import shutil
        ...
        try:
            ...
            try:
                ...
                print("  ✓ legacy PyPDF2 found")
            except ImportError:
                missing.append("pypdf")
                print("  ✗ pypdf/PyPDF2 not found")
    ...
    def show_dependency_install_dialog(self, missing):
        """Show a copyable, distribution-appropriate install command."""
        install_command, distro_label = self._build_dependency_install_command(missing)
        ...
        desc_parts = [
            "SearchBoar works without these, but installing them provides:\n"
        ]

        if "ripgrep" in missing:
            desc_parts.append("• ripgrep: much faster content searches")
        if "pdftotext" in missing:
            desc_parts.append("• Poppler/pdftotext: fastest PDF text extraction")
        if "pypdf" in missing:
            desc_parts.append("• pypdf: Python fallback for PDF text extraction")

        desc_parts.append(
            f"\nDetected system: {distro_label}"
        ...
        buffer.set_text(install_command)
        ...
        help_label.set_markup(
            '<small><i>Tip: Text is pre-selected; press Ctrl+C to copy it.</i></small>'
        )
    ...
    def extract_pdf_text(self, filepath):
        ...
        # Fallback to Python PDF library. Prefer modern pypdf, but retain
        # compatibility with systems that still provide legacy PyPDF2.
        try:
            self.log(1, "DEBUG: Trying pypdf/PyPDF2 fallback...")
            try:
            ...
            total_text = '\n'.join(text_content)
            self.log(1, f"DEBUG: {pdf_library_name} total: {len(total_text)} characters")
            return total_text
        except Exception as e:
            self.log(1, f"DEBUG: pypdf/PyPDF2 exception: {e}")
            return None
